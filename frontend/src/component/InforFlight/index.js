...
function InforFlight({ item, name, select, connect, onPriceCalculated, onSelectedValue }) {
    const [moneyAdult, setMoneyAdult] = useState(item.BusinessClass.PriceAdult);
    ...
    useEffect(() => {
        ...
        onSelectedValue?.(selectedValue); // Set default selected seat
    }, []);

    //change value total when click add or subtract quantity
    useEffect(() => {
        const newTotal = moneyAdult * value1 + moneyChildren * value2 + moneyBaby * value3;
        setTotal(newTotal);
        onPriceCalculated?.(newTotal);
    ...
    const handleSelect = () => {
        ...
        localStorage.removeItem('inforConnectFlight');
        localStorage.removeItem('isConnectFlight');
    };
            ...
            {connect ? (
                <>
                    <div className="d-flex align-items-center w-100">
                        <img
                            src={airlineLogos[item.AirlineCode]}
